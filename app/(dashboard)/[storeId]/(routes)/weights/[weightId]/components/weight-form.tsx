...
export const WeightForm: React.FC<WeightFormProps> = ({
    ...
    const onSubmit = async (data: WeightFormValues) => {
        try {
            setLoading(true);
            if (initialData) {
                await axios.patch(`/api/${params.storeId}/weights/${params.weightId}`, data);
            } else {
    ...
    const onDelete = async () => {
        try {
            ...
            toast.error("Make sure to remove all products using this weight first");
        } finally {
            setLoading(false);
            setOpen(false);
        }
                        ...
                        <FormField 
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormControl>
                                        <Input disabled={loading} placeholder="Weight value" {...field}/>
                                    </FormControl>
                                    <FormMessage />
                                </FormItem>
                            )}
